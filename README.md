...
4. [Boot from created USB](https://ubuntu.com/tutorials/install-ubuntu-desktop#4-boot-from-usb-flash-drive)
   ...
   - Mac-specific instructions:
		- Instructions and setup sprcifics will vary depending on the host version of Mac hardware. It is strongly suggested to use a pc-based host. Apple hardware is often quite proprietary, and requires many workarounds. If you have an M1 mac, your best bet is to either install inside a virtual machine, or get a loaner machine. LM will support your Mac install to the best of our ability, but our resources are quite limited. Proceed at your own risk.
		1. After creation of the live USB, shutdown your cmoputer. Reboot it while holding `CMD + R`.
		2. The Recovery Menu will load. Select your user from the list and enter your password.
		3. On the top menubar, click "Utilities -> Startup Security Utility"
		4. Set the Secure Boot to "No Security", and Allow Boot Media to "Allow booting from external or removable media". Close the window.
		5. In the Apple menu, choose shutdown.
		6. Press the power button to turn on your computer while holding down `Option`. Continue to hold until a boot menu appears. You will have the default MacOS option on the left, then two gold "EFI" options. One of these will allow you to boot into Linux. If you get a message explaining that you need to install MacOS on the drive, follwo the instructions above to reboot your machine again, and choose the other gold option.
		- Depending on your hardware, you may not be able to use the keyboard/mouse. You will need an external, wired, mouse and keyboard to complete the install. This may be rectified later by installing kernal modules, but is beyond the scope of our support. [Here](https://discussions.apple.com/thread/252029654?page=2) is a reference which may help. 
