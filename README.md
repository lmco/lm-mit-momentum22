...
4. [Boot from created USB](https://ubuntu.com/tutorials/install-ubuntu-desktop#4-boot-from-usb-flash-drive)
   ...
   - Mac-specific instructions:
	...
	- After creation of the live USB, shutdown your cmoputer. Reboot it while holding `CMD + R`.
	- The Recovery Menu will load. Select your user from the list and enter your password.
	- On the top menubar, click "Utilities -> Startup Security Utility"
	- Set the Secure Boot to "No Security", and Allow Boot Media to "Allow booting from external or removable media". Close the window.
	- In the Apple menu, choose shutdown.
	- Press the power button to turn on your computer while holding down `Option`. Continue to hold until a boot menu appears. You will have the default MacOS option on the left, then two gold "EFI" options. One of these will allow you to boot into Linux. If you get a message explaining that you need to install MacOS on the drive, follwo the instructions above to reboot your machine again, and choose the other gold option.
